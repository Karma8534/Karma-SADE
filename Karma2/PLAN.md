...
│
CLOUD ──────────── Deep reasoning tier. Anthropic API. Complex tasks only ($cost).
│
...
ORCHESTRATOR (hub-bridge + regents)
  ...
  ├── Cloud: Anthropic API → complex reasoning ($cost)
...
4. **Fallback chain:** K2 cortex → P1 cortex → Anthropic cloud → degraded spine-only.
...
| Anthropic API | CLOUD | Deep reasoning voice (complex tasks only) |
