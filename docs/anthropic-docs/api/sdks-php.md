...
However, builders are also provided, e.g., `(new Base64ImageSource)->withData("U3RhaW5sZXNzIHJvY2tz")`.
...
When the library is unable to connect to the API, or if the API returns a non-success status code, a subclass of `Anthropic\Core\Exceptions\APIException` is thrown:
...
// make additional network requests to fetch items from all pages
...
You can send undocumented parameters to any endpoint:
