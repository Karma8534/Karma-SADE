...
| **CLAUDE.md files** | Session start, when `settingSources` is enabled | Full content in every request (but prompt-cached) |
...
When the loop ends, the `ResultMessage` tells you what happened and gives you the output.
...
The `result` field is only present on the `success` variant, so always check the subtype before reading it.

## Hooks

[Hooks](/docs/en/agent-sdk/hooks) are callbacks that fire at specific points in the loop:
