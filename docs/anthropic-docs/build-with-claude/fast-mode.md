...
> **Note**: Fast mode is in beta (research preview). Join the waitlist to request access. Availability is limited while Anthropic gathers feedback.
...
When fast mode rate limits are exceeded, the API returns a `429` error with a `retry-after` header. The Anthropic SDKs automatically retry these requests up to 2 times by default.

### Falling back to standard speed

If you'd prefer to fall back to standard speed rather than wait for fast mode capacity, catch the rate limit error and retry without `speed: "fast"`.
