...
1. In the Claude Console, go to **Settings > Workspaces**.
...
Programmatically manage workspaces using the Admin API. Admin API endpoints require an Admin API key (starting with `sk-ant-admin...`).
...
API keys are scoped to a specific workspace. Resources scoped to workspaces include:
...
1. **Plan your workspace structure** before creating workspaces.
2. **Use meaningful names** — e.g., "Production - Customer Chatbot", "Dev - Internal Tools".
3. **Set appropriate limits** to prevent unexpected costs.
4. **Audit access regularly** to ensure only appropriate users have access.
5. **Monitor usage** using the Usage and Cost API.

## FAQ

**What's the Default Workspace?**
Every organization has a "Default Workspace" that cannot be edited, renamed, or removed. Usage attributed to it shows a `null` value for `workspace_id` in API responses.

**Are there limits on workspaces?**
Yes, maximum 100 workspaces per organization. Archived workspaces do not count towards this limit.

**How do organization roles affect workspace access?**
Organization admins automatically get Workspace Admin role in all workspaces. Organization billing members automatically get the Workspace Billing role. Organization users and developers must be manually added to each workspace.
