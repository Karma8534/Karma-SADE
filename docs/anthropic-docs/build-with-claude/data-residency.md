---
source: https://platform.claude.com/docs/en/build-with-claude/data-residency
scraped: 2026-03-23
...
The `inference_geo` parameter is only available on the Claude API (1P). On third-party platforms (AWS Bedrock, Google Vertex AI), the inference region is determined by the endpoint URL or inference profile. The `inference_geo` parameter is also not available via the OpenAI SDK compatibility endpoint.
...
This pricing applies to the Claude API (1P) only.

If you use Priority Tier, the 1.1x multiplier for US-only inference also affects how tokens are counted against your Priority Tier capacity. Each token consumed with `inference_geo: "us"` draws down 1.1 tokens from your committed TPM.
...
If your organization previously opted out of global routing to keep inference in the US, your workspace has been automatically configured with `allowed_inference_geos: ["us"]` and `default_inference_geo: "us"`. No code changes are required.
...
- **Per-request control:** The `inference_geo` parameter lets you specify `"us"` or `"global"` on each API call.
...
Update your workspace's inference geo settings to include `"global"` in the allowed geos and set `default_inference_geo` to `"global"`.
