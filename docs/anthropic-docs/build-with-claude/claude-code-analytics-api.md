...
- **estimated_cost.currency**: Currency code (currently always `USD`)
...
- **AI tool comparison**: Export metrics to compare Claude Code with other AI coding tools
...
Data typically appears within 1 hour of user activity completion. Only data older than 1 hour is included in responses.

**Can I get real-time metrics?**
No, this API provides daily aggregated metrics only.

**How are users identified in the data?**
Users are identified through the `actor` field:
- **`user_actor`**: Contains `email_address` for users who authenticate via OAuth
...
Tool acceptance rate = `accepted / (accepted + rejected)` for each tool type.
