...
**Sonnet 4.6** continues to support the `interleaved-thinking-2025-05-14` beta header for use with manual extended thinking (`thinking: {type: "enabled"}`).
...
Opus 4.6 may produce slightly different JSON string escaping in tool call arguments. Standard JSON parsers handle these differences automatically. If you parse tool call `input` as a raw string rather than using `json.loads()` or `JSON.parse()`, verify your parsing logic still works.
...
- **Adaptive thinking**: Learn how to use adaptive thinking mode at [/docs/en/build-with-claude/adaptive-thinking](/docs/en/build-with-claude/adaptive-thinking)
- **Models overview**: Compare all Claude models at [/docs/en/about-claude/models/overview](/docs/en/about-claude/models/overview)
- **Compaction**: Explore server-side context compaction at [/docs/en/build-with-claude/compaction](/docs/en/build-with-claude/compaction)
- **Fast mode**: Faster output token generation for Opus models at [/docs/en/build-with-claude/fast-mode](/docs/en/build-with-claude/fast-mode)
- **Migration guide**: Step-by-step migration instructions at [/docs/en/about-claude/models/migration-guide](/docs/en/about-claude/models/migration-guide)
