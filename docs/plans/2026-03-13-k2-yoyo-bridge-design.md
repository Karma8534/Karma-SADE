# K2 Yoyo Bridge — Design

**Date:** 2026-03-13
**Status:** Approved
**Scope:** Extend `fetchK2WorkingMemory()` to include yoyo state in Karma's context

## Problem

Yoyo v5 autonomous loop runs on K2 (devstral/Ollama). Karma's chat brain (Haiku/hub-bridge) has no visibility into what yoyo is doing. Karma can't see her own autonomous evolution — backlog, results, failures.

## Design

Extend the existing `fetchK2WorkingMemory()` shell command (server.js line 322) to also read:
- `yoyo_state.json` — cycle counts, success/failure rate
- `tail -20 yoyo_journal.jsonl` — full JSON entries (issue, action, result, verification)
- `yoyo_issues.jsonl` — current backlog

All files live at `/mnt/c/dev/Karma/k2/cache/` on K2.

## Changes

1. **server.js `fetchK2WorkingMemory()` shell command** — append reads for 3 yoyo files
2. **`K2_WORKING_MEM_MAX_CHARS`** — bump 4000 → 6000
3. **Section header** — update to reflect yoyo inclusion
