...
async function fetchK2WorkingMemory() {
  ...
  try {
    // F-5: cmd now includes EVOLUTION JOURNAL; F-4: patterns sorted by momentum desc
    const cmd = `${K2_FRESHNESS_CMD}; echo '=== SCRATCHPAD ===' && cat /mnt/c/dev/Karma/k2/cache/scratchpad.md 2>/dev/null || echo '(empty)' && echo '=== SHADOW ===' && tail -c 1500 /mnt/c/dev/Karma/k2/cache/shadow.md 2>/dev/null || echo '(empty)' && echo '=== KIKI STATE ===' && cat /mnt/c/dev/Karma/k2/cache/kiki_state.json 2>/dev/null || echo '(empty)' && echo '=== KIKI JOURNAL (last 5) ===' && tail -5 /mnt/c/dev/Karma/k2/cache/kiki_journal.jsonl 2>/dev/null || echo '(empty)' && echo '=== KIKI BACKLOG ===' && cat /mnt/c/dev/Karma/k2/cache/kiki_issues.jsonl 2>/dev/null || echo '(empty)' && echo '=== BEHAVIORAL PATTERNS (by momentum) ===' && python3 -c 'import json,os;f="/mnt/c/dev/Karma/k2/cache/vesper_identity_spine.json";s=json.load(open(f)) if os.path.exists(f) else {};st=s.get("evolution",{}).get("stable_identity",[]);bp=[p for p in st if p.get("type") not in ("cascade_performance",)];bp_s=sorted(bp,key=lambda p:float(p.get("momentum",1.0)),reverse=True);[(print("["+p.get("type","")+"] m="+str(round(float(p.get("momentum",1.0)),2))+" "+((p.get("proposed_change") or {}).get("description","-")))) for p in bp_s[-5:]] if bp_s else print("(none yet)")' 2>/dev/null || echo '(unavailable)' && echo '=== EVOLUTION JOURNAL (last 10) ===' && tail -10 /mnt/c/dev/Karma/k2/cache/vesper_governor_audit.jsonl 2>/dev/null || echo '(empty)'`;
    const res = await fetch(`${ARIA_URL}/api/exec`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
