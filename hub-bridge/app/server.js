...
const TOOL_DEFINITIONS = [
  ...
  {
    name: "bash",
    description: "Execute a shell command. Default: droplet. Set host=P1 for Colby's workstation (PAYBACK, Windows) or host=K2 for local worker (WSL/Ubuntu Linux, user=karma). POLICY: Only use P1 or K2 when Colby explicitly asks. P1 uses Windows paths (C:\\), K2 uses Linux paths (/home/karma/). Timeout: 30s droplet, 60s P1.",
    input_schema: {
      type: "object",
      properties: {
        command: { type: "string", description: "Shell command to execute" },
        host: { type: "string", enum: ["droplet", "P1", "K2"], description: "Target machine. droplet (default) or P1 (Colby's workstation). Only use P1 or K2 when explicitly asked." },
      },
      required: ["command"],
    },
  },
      ...
      try {
        ...
        for (let i = lines.length - 1; i >= 0 && entries.length < limit; i--) {
          try {
            ...
            if (!capturedAt) continue; // Skip entries without timestamp
