...
# === KARMA-SPECIFIC EXTENSIONS (nexus 5.6.0) ===

# Karma entity codes — pre-loaded so compress() recognizes family
KARMA_ENTITIES = {
    "Colby": "COL",
    "Julian": "JUL",
    "Karma": "KAR",
...
# Karma-specific flag
_FLAG_SIGNALS["pitfall"] = "PITFALL"
_FLAG_SIGNALS["blocker"] = "PITFALL"
_FLAG_SIGNALS["regression"] = "PITFALL"


def karma_dialect() -> "Dialect":
    """Return a Dialect instance pre-loaded with Karma entity codes."""
    return Dialect(entities=KARMA_ENTITIES)


def compress_for_cortex(text: str, max_tokens: int = 2000, metadata: dict = None) -> str:
    """
    Compress text for K2 cortex injection (32K context window).
    Fits within max_tokens budget. Uses Karma entity codes.

    Args:
        text: Raw text (e.g., MEMORY.md content)
        max_tokens: Maximum tokens in output (~3 chars per token for AAAK)
        metadata: Optional metadata dict

    Returns:
        AAAK-compressed string within token budget
    """
    d = karma_dialect()
    compressed = d.compress(text, metadata=metadata)

    # Enforce token budget (1 AAAK token ~ 3 chars)
