<#
.SYNOPSIS
    Karma SADE Sentinel - Health Monitoring Script v1.2.0
.DESCRIPTION
    Monitors Ollama and system resources (disk).
    Logs results to sentinel-runtime.log and sentinel-latest.json

    v1.2.0 changes:
    - Removed Open WebUI and Cockpit monitoring (not part of Family architecture)
    - Ollama + Disk are the only relevant P1 health checks
.NOTES
...
# Function to check if process is running (diagnostic only)
...
# Output for console
Write-Host "=== Karma SADE Sentinel Report v1.2.0 ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Service Health (HTTP):"
Write-Host "  Ollama: $($ollamaHttp.status)"
Write-Host ""
Write-Host "Process Info (diagnostic):"
