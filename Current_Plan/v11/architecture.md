...
## K2 Agency Layer (Session 84)

K2 (192.168.0.226 / 100.75.109.92 Tailscale) runs aria.service — Karma's local compute half.

### shell_run Tool
- **hub-bridge tool**: `shell_run(command)` — hub-bridge-native (not proxied to karma-server)
- **Implementation**: hub-bridge POSTs `{command}` to `K2:7890/api/exec` with `X-Aria-Service-Key` header
- **K2 endpoint**: `POST /api/exec` in aria.py — `subprocess.run(command, shell=True, timeout=30)`; returns stdout/stderr/exit_code
- **Auth**: `X-Aria-Service-Key` header. No hooks.py update needed (hub-bridge-native tool)
- **SSH NOT used from hub-bridge**: container has no SSH client. Aria /api/exec is the bridge.

### K2 Redundancy Cache
- **Location**: `/mnt/c/dev/Karma/k2/cache/` on K2 WSL
- **Pull cron** (`0 */6 * * *`): rsync identity/invariants/direction/corrections from vault-neo; tail -500 ledger
- **Push cron** (`0 * * * *`): POST k2_local_observations.jsonl to hub.arknexus.net/v1/ambient; capture token fetched live from vault-neo per push
- **aria.service drop-in**: `/etc/systemd/system/aria.service.d/10-aria-env.conf` — `Environment=HOME=/home/karma` REQUIRED or flask import fails

### Reverse Tunnel (vault-neo:2223 → K2:22)
- **User**: ALWAYS `karma@localhost` — NOT `neo@localhost` (wrong user, Permission denied)
- **Test**: `ssh vault-neo "ssh -p 2223 -l karma localhost 'whoami'"` → `karma`
