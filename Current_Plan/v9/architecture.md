...
- **Tool-calling** (Session 66): GLM-4.7-Flash gets tool-calling via `callGPTWithTools()`. Active tools (deep mode only): `graph_query(cypher)` → proxied to karma-server; `get_vault_file(alias)` → handled directly in hub-bridge using /karma/ volume mount; `write_memory(content)` → handled in hub-bridge via pending_writes Map (Session 68); `fetch_url(url)` → handled directly in hub-bridge, fetches full page text (8KB cap, HTML stripped, Session 69). Stale tools removed Session 69: `read_file`, `write_file`, `edit_file`, `bash` (had no handler, caused confabulation). `hooks.py ALLOWED_TOOLS` must be updated when adding proxied tools (not needed for hub-bridge-native handlers).
