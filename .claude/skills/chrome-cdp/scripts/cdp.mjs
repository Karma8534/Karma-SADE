...
const SOCK_PREFIX = process.platform === 'win32' ? `\\\\.\\pipe\\cdp-` : '/tmp/cdp-';
const PAGES_CACHE = process.platform === 'win32' ? resolve(process.env.TEMP || '/tmp', 'cdp-pages.json') : '/tmp/cdp-pages.json';

function sockPath(targetId) { return process.platform === 'win32' ? `\\\\.\\pipe\\cdp-${targetId}` : `${SOCK_PREFIX}${targetId}.sock`; }

function getWsUrl() {
  // Support CDP_PORT_FILE env override, then platform-specific paths
  const candidates = [
    process.env.CDP_PORT_FILE,
    process.platform === 'win32' && process.env.LOCALAPPDATA && resolve(process.env.LOCALAPPDATA, 'Google/Chrome/User Data/DevToolsActivePort'),
    process.platform === 'win32' && resolve(homedir(), 'AppData/Local/Google/Chrome/User Data/DevToolsActivePort'),
  ...
  for (const p of candidates) {
    try { readFileSync(p); portFile = p; break; } catch {}
  }
