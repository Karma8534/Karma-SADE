...
class ProposalGenerator:
    """Generate actionable proposals from consciousness observations."""

    def __init__(self):
        self.proposal_templates = {
            "graph_gap": {
                "title": "Add Missing Entity Relationships",
                "content_fmt": "Current graph state: {state}. Identified gap: {gap}. Recommendation: {reasoning}"
            },
            "performance": {
                "title": "Optimize Consciousness Cycle Performance",
                "content_fmt": "Performance observation: {gap}. Context: {state}. Action: {reasoning}"
            },
            "tooling": {
                "title": "Extend Consciousness Tooling",
                "content_fmt": "Missing capability: {gap}. Why needed: {reasoning}. Prerequisites: {state}"
            }
        }

    def generate(self, gap_identified, reasoning, current_state=None, proposal_type="graph_gap"):
        """
        Generate a proposal from identified gap.

        Args:
            gap_identified: String describing the gap
            reasoning: String explaining why it matters
            current_state: Dict of current system state (for context)
            proposal_type: One of "graph_gap", "performance", "tooling"

        Returns:
            Dict proposal object ready for JSONL append
        """
        template = self.proposal_templates.get(proposal_type, self.proposal_templates["graph_gap"])
...
class ConsciousnessLoop:
    ...
    def __init__(self, get_falkor_fn, get_graph_stats_fn, get_openai_client_fn,
        ...
        self.recent_feedback: dict = {}  # Feedback from Claude Code on proposals
    ...
    def _observe(self) -> Optional[dict]:
        ...
        # Read feedback from collab.jsonl (OBSERVE phase: gather all context)
        self.recent_feedback = self._read_feedback_from_collab()
        ...
        return {
            ...
            'has_feedback': len(self.recent_feedback) > 0
        }
    ...
    def _read_feedback_from_collab(self, collab_path: str = None) -> dict:
        """
        Read feedback entries from collab.jsonl for proposals.

        Filters for entries with type == "proposal_feedback" and extracts
        feedback keyed by proposal_id.

        Args:
            collab_path: Path to collab.jsonl (defaults to config value)

        Returns:
            Dict mapping proposal_id → {decision, reasoning, ...}
            Returns empty dict if file doesn't exist or has no feedback.
        """
        if collab_path is None:
            collab_path = os.getenv(
                "COLLAB_JSONL_PATH",
                "/opt/seed-vault/memory_v1/ledger/collab.jsonl"
            )
            ...
            with open(collab_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        logger.debug(f"Skipped malformed JSON in collab.jsonl: {line[:100]}")
                        continue

                    # Filter for proposal_feedback type only
                    if entry.get("type") != "proposal_feedback":
                        continue

                    # Extract proposal_id as key
                    proposal_id = entry.get("proposal_id")
                    if not proposal_id:
                        logger.debug("Feedback entry missing proposal_id, skipping")
                        continue

                    # Store feedback entry
                    feedback_dict[proposal_id] = {
                    ...
                    logger.debug(f"[FEEDBACK] Read feedback for {proposal_id}: {entry.get('decision')}")

        except Exception as e:
            logger.error(f"Error reading feedback from collab.jsonl: {e}")
...
Reason about both the new activity and the current state of Karma's knowledge base."""
            ...
            if self._router:
                ...
                # Proposal generation: if observation reveals a gap, synthesize proposal
                try:
                    # Detect if response contains indicators of a gap
                    gap_indicators = ["gap", "missing", "need", "should add", "recommend"]
                    if any(indicator in response_text.lower() for indicator in gap_indicators):
                        gap_identified = observation.get("episode_summaries", "Unknown gap")[:100]
                        proposal = self.proposal_generator.generate(
                            gap_identified=gap_identified,
                            reasoning=response_text[:500],  # Use first 500 chars of insight as reasoning
                            current_state=self._last_graph_stats or {},
                            proposal_type="graph_gap"
                        )
                        self._write_proposal_to_collab(proposal)
                except Exception as e:
                    logger.error(f"[PROPOSAL] Generation failed in _think: {e}")
...
"""
    ...
    def _reflect(self, cycle_num: int, cycle_ms: float, is_idle: bool, action: str):
        ...
        # Log this cycle to consciousness ledger
    ...
    def _write_proposal_to_collab(self, proposal: dict) -> bool:
        """Append proposal to collab.jsonl ledger.

        Args:
            proposal: Dict proposal object from ProposalGenerator
        ...
        try:
            ...
            with open(collab_path, "a") as f:
                f.write(json.dumps(proposal) + "\n")
            logger.info(f"[PROPOSAL] Written: {proposal['id']}")
            self.metrics["proposals_written"] += 1
            return True
        except Exception as e:
            logger.error(f"[PROPOSAL] Write failed: {e}")
            self.metrics["errors"] += 1
            return False
    ...
    def _write_consciousness_log(self, entry: dict) -> None:
        ...
        CONSCIOUSNESS_JSONL = "/ledger/consciousness.jsonl"
        try:
